...
function is_running(string $gw_name): bool
{
    ...
    return $pid > 0 && file_exists("/proc/$pid") || (shell_exec("kill -0 $pid 2>/dev/null; echo $?") == "0
");
}
...
function stop_instance(string $gw_name): void
{
    ...
    if (file_exists($pidfile)) {
        $pid = (int)trim(file_get_contents($pidfile));
        if ($pid > 0) exec("kill " . (int)$pid . " 2>/dev/null");
    }

    // Небольшая пауза и принудительное завершение
    usleep(500000);
    exec("pkill -f " . escapeshellarg("gw_monitor_probe.py {$sockfile}") . " 2>/dev/null");
